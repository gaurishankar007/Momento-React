  ...
  const userLogin = (e) => {
    ...
    axios.post(`${REACT_APP_BASE_URL}user/login`, userData).then((result) => {
      if (result.data.token) {
        if (
          result.data.userData.admin === false &&
          result.data.userData.superuser === false
        ) {
        ...
        <div className="register-user-form px-4 py-3">
          ...
          <form>
            ...
            <div className="form-group mb-3">
              ...
              />
              <small id="passwordHelp" className="form-text ms-1">
            ...
            <div className="d-flex justify-content-center">
              <Link className="btn lR-button" to="/user-registration">
                Create an account
              </Link>
            </div>
          </form>
