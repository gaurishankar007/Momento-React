...
router.post("/chat/access", auth.verifyUser, async (req, res)=> {
    ...
    if(findChat.length > 0) {
        ...
        try {            
            ...
            const fullChat = await chat.findOne({_id: createChat._id}).populate("users", "+ username profile_pic email");
...
router.post("/groupChat/create", auth.verifyUser, async (req, res)=> {
    if(!req.body.users || !req.body.name) {
        return res.status(400).send({message: "please fill up all of the fields at first."})
    }

    var users = req.body.users;
    if(users.length < 2) {
        return res.status(400).send("Requires more than 2 users to form a group chat.")
    }
